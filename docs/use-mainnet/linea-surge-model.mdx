...
This multiplier will be a constant over the time period of a single Volt (1 month) and will reduce over Volts. This is used to reward users that provide liquidity on Linea since the mainnet went Live and users that will bridge active liquidity to Linea after the announcement but before The Surge program starts.
